...
export const DAPPView = (_props: DAPPViewProps) => {
  const { sdk, provider: ethereum, status, chainId, account, balance, readOnlyCalls, connected } = useSDK();
  ...
  const testEthers = async () => {
    ...
    const web3Provider = new ethers.providers.Web3Provider(ethereum as unknown as ethers.providers.ExternalProvider);
  ...
  return (
    <View style={{ borderWidth: 2, padding: 5 }}>
      <Text style={styles.title}>
        {sdk?._getDappMetadata()?.name} ({connected ? "connected" : "disconnected"})
      </Text>

      {readOnlyCalls && <Button title="Test read-only calls" onPress={testReadOnlyCalls} />}
      ...
      {connected ? (
        <>
          ...
          <Text style={textStyle}>{response ? `Last request response: ${response}` : ""}</Text>
        </>
      ) : (
