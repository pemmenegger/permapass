...
export class StorageManagerRN implements StorageManager {
  ...
  constructor({ debug }: StorageManagerProps | undefined = { debug: false, enabled: true }) {
    if (debug) {
      this.debug = debug;
    }
  }
  ...
  public async persistChannelConfig(channelConfig: ChannelConfig) {
    ...
    await AsyncStorage.setItem(STORAGE_PATH, payload, () => {
      if (this.debug) {
        console.debug("StorageManagerRN::persisChannelConfig() saved to storage.", AsyncStorage);
      }
    });
  ...
  public async getPersistedChannelConfig(): Promise<ChannelConfig | undefined> {
    ...
    if (this.debug) {
      console.debug("StorageManagerRN::getPersistedChannelConfig()", AsyncStorage);
    }
    ...
    if (this.debug) {
      console.debug("StorageManagerRN::getPersisChannel channelConfig", channelConfig);
    }
